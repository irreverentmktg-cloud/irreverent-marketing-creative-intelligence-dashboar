import { Target, Users, CheckCircle, XCircle, Megaphone, ArrowRight } from "lucide-react";
...
const voice = {
  ...
  examples: [
    {
      do: "Your competitors are already 3x ahead. Here's how to catch up.",
      dont: "We offer a comprehensive solution for improving your marketing outcomes.",
    },
    {
      do: "Stop wasting money on ads that don't convert. We fixed that.",
      dont: "Our platform helps businesses optimize their advertising spend efficiently.",
    },
    {
      do: "This ad made $47K in 72 hours. Here's the exact framework.",
      dont: "Discover how our solution can help you achieve better advertising results.",
    },
  ],
};
...
const personas = [
  {
    ...
    age: "28–38",
    description:
      "Entrepreneurial, scrappy, obsessed with growth metrics. Responds to direct ROI claims and social proof from people like them.",
  ...
  },
  {
    ...
    age: "35–50",
    description:
      "Marketing director or in-house operator. Needs to justify spend to leadership. Responds to benchmarks and clear attribution.",
  ...
  },
  {
    ...
    age: "22–32",
    description:
      "Content creator monetising their audience. Responds to authenticity, community, and simple tools.",
          ...
          <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {voice.tone.map((t) => (
                <span
                  key={t}
                  className="text-sm px-3 py-1 rounded-full bg-green-500/10 text-green-300 border border-green-500/20"
                >
                  {t}
                </span>
              ))}
          ...
          <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {voice.avoid.map((t) => (
                <span
                  key={t}
                  className="text-sm px-3 py-1 rounded-full bg-red-500/10 text-red-300 border border-red-500/20"
                >
                  {t}
                </span>
              ))}
        ...
        >
          {personas.map((persona) => (
            <div
              ...
              className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-5 flex flex-col"
            >
              <div className="flex items-center gap-2 mb-3">
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {persona.triggers.map((t) => (
                    <span
                      key={t}
                      className="text-xs px-2 py-0.5 rounded bg-violet-500/10 text-violet-300 border border-violet-500/20"
                    >
                      {t}
                    </span>
                  ))}
              ...
              <div className="mt-auto pt-3 border-t border-violet-500/10">
                <Link
                  href="/briefs"
                  data-testid={`persona-brief-cta-${persona.slug}`}
                  className="flex items-center gap-1.5 text-xs text-violet-400 hover:text-violet-300 font-medium transition-colors"
                >
                  Create brief for this persona
                  <ArrowRight size={11} />
                </Link>
              </div>
            </div>
